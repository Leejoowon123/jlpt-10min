...
### 옵션 B — Node `npx` (네트워크 다운로드 1회 필요)
```powershell
cd D:\TEST
npx serve -l 5173 .
```

### 옵션 C — VSCode Live Server 등 정적 서버
루트로 `D:\TEST` 를 지정하면 됨.

### 테스트
```powershell
# 단위 / 로직 회귀
node D:\TEST\smoke.mjs

# 브라우저(DOM) QA — jsdom 기반. 최초 1회만 의존성 설치 필요.
npm install --no-save jsdom
node D:\TEST\qa.mjs
```
`qa.mjs` 는 실제 DOM 을 띄워 다음을 자동 검증한다:
선택 전 차단(번역/연상카드/해설) · 선택 후 노출 · favBtn 단어 한정 · 오답→실패노트 ·
청해 TTS 미지원/지원 시 스크립트 토글 · **오늘의 10분 완료 화면이 이번 회차만 집계** ·
문법 비교 답 선택 시 학습 기록 · 모바일 폭 CSS 규칙.
...
docs/
  ...
  next-roadmap.md                  단계별 우선순위 (안정화→브라우저QA→대량2차→한자→N4→모바일)
      conversationTopics.js        오프라인 회화 주제 (필수 어휘/문법 ID, starter, repair hints)
  docs/
    offline-conversation.md        오프라인 회화 엔진 인터페이스 / STT·TTS·Evaluator 어댑터 설계
```
...
### 한자 데이터 모델 ([js/data/kanji.js](js/data/kanji.js))
...
전체 로드맵 → [docs/next-roadmap.md](docs/next-roadmap.md).
N5 라운드 상세 → [docs/n5-expansion-plan.md](docs/n5-expansion-plan.md).

**현재 위치**: N5 대량1차 완료 → 안정화 단계.
**다음 단계**: 실제 브라우저 QA ([docs/browser-qa-checklist.md](docs/browser-qa-checklist.md)) → N5 대량2차 (어휘 500).
...
6. **오프라인 AI 회화 모듈** — STT → 로컬 평가 → TTS 어댑터로 단계 결합. 설계는 [docs/offline-conversation.md](docs/offline-conversation.md).
...
- [docs/offline-conversation.md](docs/offline-conversation.md) — 오프라인 회화 모듈 어댑터 설계.
- [docs/content-authoring-guide.md](docs/content-authoring-guide.md) — **콘텐츠 작성 가이드** (라이선스/스키마/금지사항/난이도 기준/회화 재사용 태깅).
- [docs/n5-expansion-plan.md](docs/n5-expansion-plan.md) — N5 콘텐츠 2차 확장 계획 (목표 수량 / 카테고리 분배 / 라운드 진행).
...
설계 상세: [docs/offline-conversation.md](docs/offline-conversation.md).
...
본 저장소의 코드/예문/번역은 본 프로젝트용 창작. 외부 사전 데이터 미포함. 자세한 내용은 [LICENSE_NOTES.md](LICENSE_NOTES.md).
